...
def extract_keywords_for_clusters(labels, texts, num_keywords=1):
    cluster_keywords = {}
    for label in set(labels):
...
def determine_optimal_clusters(vectors):
    inertias = []
    for n_clusters in range(2, 40):  # Adjust the range as needed
        kmeans_model = KMeans(n_clusters=n_clusters)
        kmeans_model.fit(vectors)
        inertias.append(kmeans_model.inertia_)
    plt.plot(range(2, 40), inertias)
...
def calculate_similarity_accuracy(labels, texts):
    cluster_similarities = {}
    for label in set(labels):
        indices = [i for i, lbl in enumerate(labels) if lbl == label]
        cluster_texts = [texts[i] for i in indices]
        similarity_matrix = cosine_similarity(tfidf_vectors[indices], tfidf_vectors[indices])
        cluster_similarities[label] = similarity_matrix.mean()
    return cluster_similarities

cluster_similarities = calculate_similarity_accuracy(dbscan_labels, news_titles)
average_similarity = sum(cluster_similarities.values()) / len(cluster_similarities)
print(f"Average Similarity Accuracy: {average_similarity}")
